...
def upgrade() -> None:
    ...
    # Enum types
    agent_status = postgresql.ENUM('online', 'busy', 'offline', name='agent_status', create_type=False)
    job_status = postgresql.ENUM('queued', 'assigned', 'running', 'completed', 'failed', name='job_status', create_type=False)
    product = postgresql.ENUM('vehicle_gateway', 'asset_gateway', 'environmental_monitor', 'industrial_gateway', name='product', create_type=False)
    ...
    op.create_table(
        ...
        sa.Column('product', sa.Enum('vehicle_gateway', 'asset_gateway', 'environmental_monitor', 'industrial_gateway', name='product', create_constraint=True), nullable=False),
        sa.Column('status', sa.Enum('queued', 'assigned', 'running', 'completed', 'failed', name='job_status', create_constraint=True), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('online', 'busy', 'offline', name='agent_status', create_constraint=True), nullable=False),
